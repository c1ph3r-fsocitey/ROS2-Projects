...
class TurtleController(Node):
    def __init__(self):
        ...
        self.declare_parameter("turtle_speed", 2)
        ...
        if len(msg.turtles) > 0:
            if self.catch_closest_turtle_first:
                ...
                for turtle in msg.turtles:
                    dist_x = turtle.x - self.pose.x #type: ignore
                    dist_y = turtle.y - self.pose.y #type: ignore
                    distance = math.sqrt(dist_x**2 + dist_y**2)
                    if closest_turtle == None or distance < closest_turtle_distance: # type: ignore
    ...
    def control_loop(self):
        ...
        if distance > 0.5:
            # position
            msg.linear.x = self.get_parameter("turtle_speed").value * distance #type: ignore
